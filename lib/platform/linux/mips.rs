...
impl Mips {
    ...
    fn initialize(mut state: State<Mips>, elf_linker: &ElfLinker)
        ...
        let environment = Environment::new()
            ...
            .environment_variable(
                EnvironmentString::new_environment_variable("SCRIPT_NAME", 128));
    ...
    fn get_register(state: &mut State<Mips>, name: &str) -> Result<u64> {
        Ok(state.eval_and_concretize(&il::expr_scalar(name, 32))?
    ...
    fn get_stack_arg(state: &mut State<Mips>, offset: i64) -> Result<u64> {
        ...
        let value = state.memory().load(address, 32)?
            .ok_or(format!("Failed to get stack arg at address 0x{:x}", address))?;
        Ok(state.eval_and_concretize(&value)?
    ...
    pub fn syscall(mut state: State<Mips>)
        -> Result<Vec<Successor<Mips>>> {

        let syscall_num = state.eval_and_concretize(&il::expr_scalar("$v0", 32))?
        ...
        match syscall_num.value_u64().unwrap() {
            ...
            SYSCALL_GETRLIMIT => {
                ...
                if a0 == 3 { // This is RLIMIT_STACK according to angr
                    state.memory_mut().store(a1, &il::expr_const(1024*8*8, 32))?;
                    state.memory_mut().store(a1 + 4, &il::expr_const(1024*8*16, 32))?;
                    trace!("getrlimit {}", a0);
                }
                else {
            ...
            SYSCALL_MMAP => {
                ...
                trace!("mmap(0x{:x}, 0x{:x}, 0x{:x}, 0x{:x}, 0x{:x}, 0x{:x} = 0x{:x}",
                    a0, a1, a2, a3, stack0, stack1, address);
            ...
            SYSCALL_OPEN => {
                ...
                let filename = match state.get_string(a0)? {
                    Some(filename) => filename,
                    None => { bail!("Could not get filename for open"); }
                };
            ...
            SYSCALL_READ => {
                ...
                let a1 = state.eval_and_concretize(&il::expr_scalar("$a1", 32))?
                    ...
                    .value_u64()
                    .ok_or("Could not get value for a1")?;
                let a2 = state.eval_and_concretize(&il::expr_scalar("$a2", 32))?
                ...
                if let Some(bytes) = result {
                    ...
                    state.set_scalar(
                        "$v0", &il::expr_const(bytes.len() as u64, 32))?;
                    state.set_scalar("$a3", &il::expr_const(0, 32))?;
                }
                else {
                    state.set_scalar("$v0", &il::expr_const(-1i64 as u64, 32))?;
            ...
            SYSCALL_UNAME => {
                ...
                state.memory_mut().store(a0 + (65 * 2), &il::expr_const(0x33, 8))?;
            ...
            SYSCALL_WRITEV => {
                ...
                for i in 0..a2.value_u64().unwrap() {
                    ...
                    let iovec_address =
                        state.memory().load(iovec_address, 32)?
                            .ok_or(format!("Failed to get iovec_address at 0x{:x}", iovec_address))?;
                    let iovec_length =
                        state.memory().load(iovec_length, 32)?
...
impl Platform<Mips> for Mips {
    fn intrinsic(state: State<Mips>, intrinsic: &il::Intrinsic)
        -> Result<Vec<Successor<Mips>>> {

        Mips::intrinsic(state, intrinsic)
    }

    fn merge(&mut self, other: &Mips, _: &il::Expression)
        -> Result<bool> {
