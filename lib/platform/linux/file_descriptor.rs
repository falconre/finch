...
impl FileDescriptor {
    ...
    pub fn new<S: Into<String>>(fd: usize, path: S)
        -> FileDescriptor {
    ...
    pub fn read(&mut self, filesystem: &FileSystem, length: usize)
        -> Result<Vec<il::Expression>> {

        let bytes = filesystem.file_bytes(&self.path)
    ...
    pub fn write(
        ...
        data: Vec<il::Expression>
    ) -> Result<()> {
        let file_bytes =
            filesystem.file_bytes_mut(&self.path)
