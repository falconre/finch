...
impl Memory {
    ...
    pub fn set_permissions(
        ...
        if address & (PAGE_SIZE as u64 - 1) > 0 {
            ...
            for i in 0..(length / PAGE_SIZE) {
                RC::make_mut(
                    self.pages.entry(address + (i * PAGE_SIZE) as u64)
    ...
    pub fn flatten(&mut self) -> Result<()> {
        ...
        'pages_loop: for (address, _) in &self.pages {
            ...
            'page_loop: for i in 0..PAGE_SIZE {
                let byte = match self.get_byte(address + i as u64)? {
                    Some(byte) =>
                        if !byte.all_constants() {
                            ...
                            eval(&byte)?.value_u64()
                                .expect("Failed to get value_u64 in memory.flatten()") as u8
                        },
                    None => 0
                };
    ...
    fn get_byte(&self, address: u64) -> Result<Option<il::Expression>> {
        ...
        match byte {
            ...
            None => {
                let constant = match self.backing()
                                         .and_then(|backing|
                                            backing.get8(address)) {
...
*/
    ...
    pub fn load_buf(&self, address: u64, len: usize)
        -> Result<Option<Vec<il::Expression>>> {

        let mut buf = Vec::new();
        for i in 0..(len as u64) {
            match self.load(address + i, 8)? {
                Some(expression) => { buf.push(expression); },
                None => { return Ok(None); }
            }
        }
...
impl TranslationMemory for Memory {
    fn permissions(&self, address: u64) -> Option<MemoryPermissions> {
        self.pages.get(&(address & (!(PAGE_SIZE as u64 - 1))))
