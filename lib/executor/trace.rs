...
impl TraceItem {
    ...
    pub fn address(&self) -> Option<u64> { self.address.clone() }
}
...
impl Trace {
    ...
    pub fn items(&self) -> &[TraceItem] { &self.items }
    ...
    pub fn push(
        ...
        self.items.push(TraceItem::new(index, program_location, address));
    }
                ...
                match operation {
                    ...
                    il::Operation::Store { src, .. } => {
                        ...
                        let cond =
                            (0..(src.bits() / 8))
                                ...
                                .any(|offset|
                                    addresses_read
                                        .contains(&(address + offset as u64)));
                        if cond {
                    ...
                    il::Operation::Intrinsic { intrinsic } => {
                        ...
                        if cond {
                            ...
                            for scalar in
                                intrinsic.scalars_read().unwrap_or(Vec::new()) {
                                scalars_read.insert(scalar.clone());
                            }
                            items.push(item.clone());
                        }
